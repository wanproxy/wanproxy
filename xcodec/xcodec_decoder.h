...
private:
	bool reference(Buffer *output, Buffer *input)
	{
		BufferSegment *seg;
		uint64_t sum;
	...
	bool unescape(Buffer *output, Buffer *input)
	{
		if (input->length() - 1 < 1)
			return (false);
		input->skip(1);
	...
	bool backreference(Buffer *output, Buffer *input)
	{
		BufferSegment *seg;
